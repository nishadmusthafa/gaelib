...
class ModelUnitTestCase(BaseUnitTestCase):
    ...
    def create_object(self,**kwargs):
        obj = SampleModel(**kwargs)
        return obj

    def test_passing_an_already_set_key_behaviour(self):
        #adding a key in the db to check with
        key = SampleModel.generate_key('set_key_behaviour')
        model = SampleModel(key = key, string1 = 'value1')
        model.put()      
    ...
    def test_key_str_behaviour(self):
        model = SampleModel(key_str = 'key_str_behaviour')
        self.assertIsNotNone(model.key(), "Failed to generate key from key_str")
    
    def test_entity_as_keyword_behaviour(self): 
        model = SampleModel(entity = self.create_object(string1 = 'value1', string2 = 'value2'))
    ...
    def test_check_update_and_put(self):
        #checking put
        key = SampleModel.generate_key('put and update')
        self.clear_database()

        model = SampleModel(key = key, string1 = 'test_set_1')
        model.put()
    ...
    def test_check_delete(self):
        key = SampleModel.generate_key('delete')
        #creatng a record to be deleted
        model = SampleModel(key = key, string1 = "This record will be deleted")
        model.put()
        model = SampleModel(key = key)
    ...
    def test_query(self):
        ...
        model = SampleModel(key_str = 'record1', string1 = '123', string2 = '456', int1 = 111)
        model.put()
        model = SampleModel(key_str = 'record2', string1 = '123', string2 = '456', int1 = 113)
        model.put()
        model = SampleModel(key_str = 'record3', string1 = '123', string2 = '456', int1 = 112)
        model.put()
        ...
        for obj in q.fetch():
            ...
            last_value = obj.int1
        self.assertEqual(len(q.fetch()), 2, "Failed to fetch. Fetched {} instead of 2 items".format(len(q.fetch())))

    def test_retrieve(self):
        model = SampleModel(key_str = 'record1', string1 = '123', string2 = '456', int1 = 111)
        model.put()
        model = SampleModel(key_str = 'record2', string1 = '123', string2 = '456', int1 = 112)
        model.put()
        model = SampleModel(key_str = 'record3', string1 = '123', string2 = '456', int1 = 113)
        model.put()
        model = SampleModel(key_str = 'record3', string1 = '123', string2 = '456', int1 = 114)
        model.put()
        ...
        self.assertEqual(items[0].int1, 111, "Failed to retrieve using key_str")
        ...
        for obj in items:
            ...
            self.assertEqual(obj.string1,'123', "Failed to apply filter")
            self.assertGreaterEqual(last_value,obj.int1, "Failed to order items")
            last_value = obj.int1
        self.assertEqual(len(items),limit, "Failed to apply limit. Fetched {} instead of {} items".format(len(items),limit))
